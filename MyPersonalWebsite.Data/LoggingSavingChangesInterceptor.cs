        ...
        CancellationToken cancellationToken = default)
        {
            ...
            foreach (var change in modifiedEntities)
            {
                ...
                else if (change.State == EntityState.Modified)
                {
                    foreach (IProperty prop in change.OriginalValues.Properties)
                    {
                        if (change.OriginalValues[prop.Name] != change.CurrentValues[prop.Name])
                        {
                            originalValuesjson.Add(prop.Name, change.OriginalValues[prop.Name]??"");
                            currentValuesjson.Add(prop.Name, change.CurrentValues[prop.Name] ?? "");
                        }
